...
void removeAtFront( Node ** head )
{
	Node *freeThisNode;
	freeThisNode = *head;
	*head = freeThisNode->next;
	freeThisNode->next = NULL;
	free(freeThisNode->word);
	free(freeThisNode);
}
...
void removeNode( Node ** head, char * key )
{
	...
	prev = *head;
	removeNode = searchNode(*head,key);
...
Node * searchNode ( Node * head, char * key )
{
	...
	temp = head;
	while(temp) {
		if(strncmp(temp->word,key,BUFFER_CAP) == 0) {
			return temp;
		}
		temp = temp->next;
	}
