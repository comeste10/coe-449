...
int main( int argc, char *argv[] )
{
	...
	/* YOUR CODE HERE
		read each value from the text file using fscanf as a short int
		and write it out to the binary file using fwrite
	*/
	...
	/* PRINT binFile to console */

	binFile = fopen( argv[2], "rb" );
	while(fread(&shortVal, sizeof(short), 1, binFile) == 1) {
		printf("%hi\n", shortVal);
	}
